...
Quark is a function, Quirk is a function.

Quark can be thought of as a wrapper around a single html element.
Quirk can be thought of as a wrapper around an array of multiple html elements.

Depending upon the string argument passed in, Quark may return a Quark function or a Quirk function.
Quirk will always return a Quirk function, a multiple input cannot produce a single output. 

Sorry if that is a bit heavy, lets create a select dropdown....

```javascript
var values = ["rain","hail","snow","fog"];
ú("body|ace,select")
	("^select|ace,option|val,1|html,%1",values[0])
	("^select|ace,option|val,2|html,%1",values[1])
	("^select|ace,option|val,3|html,%1",values[2])
	("^select|ace,option|val,4|html,%1",values[3]);
```

The default re-entry point of a quark or quirk function is the exit point of its parent.
Although the code above looks neat and ordered it could more efficiently be written as follows:

```javascript
var values = ["rain","hail","snow","fog"];
ú("body|ace,select|ace,option|val,1|html,%1",values[0])
	("^select|ace,option|val,2|html,%1",values[1])
	("^select|ace,option|val,3|html,%1",values[2])
	("^select|ace,option|val,4|html,%1",values[3]);
```

The initial context is the body element, the ace command creates the select element and the context steps down to this element.
The following ace command creates the "rain" option and the context steps down to this element.
It is only now that we have a problem, the next option wants to be a sibling of that first option not a child.  
The re-entry argument (^select) steps back up to the select element before appending the next option.

By default the re-entry points are named automatically, this can be a problem if for instance you wish to nest multiple divs within divs.
In this case you can use a set command to name a snapshot of any namespace context as follows:

```javascript
var values = ["rain","hail","snow","fog"];
var returnvalue=ú("body|ace,select|set,^olaf,~^select|ace,option|val,1|html,%1",values[0])
	("^olaf|ace,option|val,2|html,%1",values[1])
	("^olaf|ace,option|val,3|html,%1",values[2])
	("^olaf|ace,option|val,4|html,%1",values[3]);
```

Looking at the example above variable "returnvalue" is now a reference to the whole of the body-select-option structure.
"returnvalue" is a quark function.  It's exit point is the last option element created.

Therefore the code `returnvalue()` will return the html element `<option value="4">fog</option>`;
However the code `returnvalue(-1)` will return the namespace object.
For the example above this will look something like this:

* ^			: the root selector element(s), in this case `<body>`
* ^olaf		: the copy of ^select created using the set command
* ^option	: an autogenerated array of 4 option elements
* ^select	: an autogenerated reference to the select element
* a			: an array containing the compiled quark string with additional properties
* e			: the exit point context element, in this case `<option value="4">`
* xs		: a reference to the external scope, in this case the global window object.

The code `returnvalue("^")` will return a quark function with its exit point set to the root element i.e. the body tag.
The code `returnvalue("^")()` will return the actual body tag element.

## Understanding the namespace object


## Native html and native html shortcuts
