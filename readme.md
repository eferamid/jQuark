...
Quark can be thought of as a wrapper around an array of multiple html elements.
...
The initial context is the body element, the ace command creates the select command and the context steps down to this element.
...
The re-entry argument steps back up to the select element before appending the next option.
...
* ^select	: the autogenerated reference to the select element
